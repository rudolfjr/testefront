...
function Movie({ match }) {
  ...
  useEffect(() => {
    async function getFilme() {
      const response = await api.get(
        `movie/${id}?api_key=d870f3353d5cf268798d6e6a25a2dcbc&language=${language}`
      );
    ...
    async function getTrailer() {
      const response = await api.get(
        `movie/${id}/videos?api_key=d870f3353d5cf268798d6e6a25a2dcbc&language=${language}`
      );
  ...
  return (
    <>
      {filme.original_title !== undefined && filme.original_title.length ? (
        <Container>
          <Row>
            <Col xs={12} sm={4}>
              <Menu />
